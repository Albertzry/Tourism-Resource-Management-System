        ...
        <el-aside width="collapse" class="aside">
            ...
            <el-menu default-active="$route.path"
                ...
                <el-submenu index="1">
                    ...
                    <el-menu-item index="/parking" class="select">
        ...
        <el-container>
            ...
            <el-main>
                <el-card class="box-card">
                    ...
                    <el-row>
                        <el-button size="mini" type="primary" @click="add">记录入库车辆</el-button>
                        <el-button size="mini" type="primary" @click="resortByTime">按停车时间排序</el-button>
                        <input type="text" class="search" placeholder="请输入要查询的汽车的车牌号">
                        ...
                        <el-dialog :title="title" :visible.sync="dialogFormVisible" width="30%">
                            <el-form :model="form" :rules="rules" ref="form">
                                ...
                                <el-form-item label="车牌号:" prop="car_num">
                                    <el-input v-model="form.car_num" placeholder="请输入汽车车牌号" style="width:80%"></el-input>
                                </el-form-item>
                            </el-form>
                    ...
                    <el-table ref="singleTable" :data="tableData" stripe border>
                        ...
                        <el-table-column label="操作" align="center">
                            <template slot-scope="scope">
                                <el-button
                                        size="mini"
                                        @click="edit(scope.$index, scope.row)">出库
                                </el-button>
                                <el-button
                                        size="mini"
                                        type="danger"
    ...
    export default {
      ...
      data() {
        return {
          ...
          tableData: [],
          rules: {
            car_num:[{required:true,message:'请输入入库汽车车牌号',trigger:'blur'}],
          }
        }
      },
      methods: {
        ...
        remove(index, row) {
          ...
          this.$axios({
            method: 'post',
            url: 'http://localhost:9090/parking/delete/' + row.car_num,
          }).then((response) => {
            this.$message({
              message: '删除成功!',
              type: 'success'
        ...
        resortByTime(){
          this.$axios({
            method:'get',
            url:'http://localhost:9090/parking/info/sortTime'
          }).then((response)=>{
        ...
        getList() {
          this.$axios({
            method: 'get',
            url: 'http://localhost:9090/parking/info',
          }).then((response) => {
            this.tableData = response.data
            console.log(this.tableData);
          }).catch((error) => {
        ...
        search() {
          ...
          let car_num = searchContent;
          this.$axios({
            method: 'get',
            url: 'http://localhost:9090/parking/search/' + car_num,
          }).then((response=>{
            this.tableData=response.data;
            ...
            if (valid) {
              if (this.insertOredit==1) {
                this.$axios({
                  method: 'post',
                  data: this.form,
                  url: 'http://localhost:9090/parking/add',
                }).then((response) => {
                  this.$message({
                    message: '新增成功!',
                    type: 'success'
                ...
                this.$axios({
                  method: 'post',
                  url: 'http://localhost:9090/parking/leave/'+this.form.car_num,
                }).then((response) => {
                  this.$message({
                    message: '修改成功!',
                    type: 'success'
